...
mcols(acc[["ACC_Mutation-20160128"]])$Variant_Classification <- Variants
...
```{r}
resto <- qreduceAssay(ragex, gn,
    function(scores, ranges, qranges) {any(scores %in% vclass)}, "Variant_Classification")
rownames(resto) <- names(gn)
resto[is.na(resto)] <- 0
resto <- resto[!is.na(rownames(resto)), ]
head(sort(rowSums(resto), decreasing = TRUE), 25)
```
