...
```{r}
ov <- curatedTCGAData("OV", "Mutation", FALSE)
```
...
regov <- ov[[1]]
gen <- translateBuild(unique(genome(regov)))
genome(regov) <- gen
...
vclass <- lapply(mutlist, function(mutassay) {
    somas <- mcols(mutassay)$Mutation_Status == "Somatic"
    mutassay <- mutassay[somas, ]
...
rowRanges(ragov) <- unstrand(rowRanges(ragov))
## OV 

variant_fun <- function(scores, ranges, qranges)
    { any(scores != "Silent") }
genome(ragov) <- translateBuild(genome(ragov))
resov <- qreduceAssay(ragov, gn18, variant_fun, "Variant_Classification",
    background = FALSE)
rownames(resov) <- names(gn18)
resov <- resov[!is.na(rownames(resov)), ]
