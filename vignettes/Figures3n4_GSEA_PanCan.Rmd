...
download.file(
    url = "https://data.broadinstitute.org/gsea-msigdb/msigdb/release/6.2/h.all.v6.2.entrez.gmt",
    destfile = "../data/h.all.v6.2.entrez.gmt"
)
hall.gs <- getGenesets("../data/h.all.v6.2.entrez.gmt")
names(hall.gs) <- sub("^HALLMARK_", "", names(hall.gs))
```
...
res <- runEA(exp.list, methods=c("ora", "padog"),
                gs=kegg.gs, perm=c(0, 1000), save2file=TRUE, out.dir="../data/gsea/kegg")
res <- runEA(exp.list, methods=c("ora", "padog"),
                gs=go.gs, perm=c(0, 1000), save2file=TRUE, out.dir="../data/gsea/go")
res <- runEA(exp.list, methods=c("ora", "padog"), gs=hall.gs,
                perm=c(0,1000), save2file=TRUE, out.dir="../data/gsea/hall")
res <- lapply(res$ora, function(r) r$ranking)
```

```{r}
res <- readResults(data.dir="../data/gsea/hall", data.ids=names(exp.list),
...
hall.ora <- isEnriched(res$ora)
hall.padog <- isEnriched(res$padog, nr.top=rownames(hall.ora))

rownames(hall.ora) <- rownames(hall.padog) <- hall.i2n[rownames(hall.ora)]

h1 <- Heatmap(hall.ora, cluster_rows=FALSE, cluster_columns=FALSE,
                show_row_names=FALSE, show_heatmap_legend = FALSE,
                col=c("white", "black"), column_title="ORA p < 0.05")
h2 <- Heatmap(hall.padog, cluster_rows=FALSE, cluster_columns=FALSE,
