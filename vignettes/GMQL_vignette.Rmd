...
```{r}
rnaexp <- assay(brca1[["BRCA_RNASeq2GeneNorm-20160128_ranged"]]) > 3
```
...
TODO: resolve replicates in other assays
...
rnamerged <- mergeReplicates(brca2[, , "BRCA_RNASeq2GeneNorm-20160128_ranged"],
    replicates = replicated(brca2[, , "BRCA_RNASeq2GeneNorm-20160128_ranged"]),
    simplify = function (x) {
        gt3 <- x > 3
        if (any(gt3))
...
brca2[["BRCA_RNASeq2GeneNorm-20160128_ranged"]] <- rnamerged[[1]]
...
keep <- assay(brca2, "BRCA_RNASeq2GeneNorm-20160128_ranged") > 3 &
  assay(brca2, "has.meth") &
  assay(brca2, "mutations")
```
