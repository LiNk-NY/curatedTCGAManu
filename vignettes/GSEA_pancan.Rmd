...
First, download the 'altRNA' branch of RTCGAToolbox.

```{r}
## remotes::install_github("LiNk-NY/RTCGAToolbox", ref = "altRNA")
library(RTCGAToolbox)
dl <- c("COAD", "READ", "UCEC")
redl <- lapply(setNames(dl, dl), function(x) {
    tx <- getFirehoseData(x, RNASeq2GeneNorm = TRUE, clinical = FALSE)
    biocExtract(tx, "RNASeq2GeneNorm")
    }
)
exper <- as(unlist(redl), "ExperimentList")
remae <- MultiAssayExperiment(experiments = exper)
sampleTables(remae)
```
