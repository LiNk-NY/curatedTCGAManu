...
redl <- lapply(setNames(dl, dl), function(x) {
    ...
    newassay <- newassay[, !duplicated(colnames(newassay))]
...
ccids$sample %in% colnames(exper[[2]])
ccids$sample %in% unlist(colnames(exper[1:2]))
reids$sample %in% unlist(colnames(exper[3:4]))
ceids$sample %in% unlist(colnames(exper[5:6]))
```
...
core <-
    exper[ c("COAD.illuminahiseq", "READ.illuminahiseq", "UCEC.illuminahiseq") ]
